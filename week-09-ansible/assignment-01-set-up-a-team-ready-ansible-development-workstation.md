...
Ansible should be installed inside a Python virtual environment because it keeps Ansible and its Python dependencies isolated from the system Python installation. This helps prevent version conflicts and makes it easier to reproduce the same development environment on another machine.
...
Keeping private SSH keys off version control protects sensitive access credentials from being exposed, while excluding the .venv/ folder avoids bloating the repository with machine-specific binary files that are easily recreated locally
